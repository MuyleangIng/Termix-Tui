...
func Load(path string) (Config, error) {
	...
	v.SetDefault("font_stack", []string{"MesloLGM Nerd Font", "monospace"})
...
activity_height: %d
border_style: %q
`, cfg.HomeDir, yamlList(cfg.ThemeDirs), themeName, shellName, fontName, yamlList(firstNonEmptyList(cfg.FontStack, []string{fontName, "monospace"})), yamlList(cfg.CustomFonts), normalizeActivityHeight(cfg.ActivityHeight), normalizeBorderStyle(cfg.BorderStyle))
...
func SaveFontChoice(cfg Config, fontName string) error {
	...
	cfg.FontStack = prependUnique(fontName, firstNonEmptyList(cfg.FontStack, []string{"MesloLGM Nerd Font", "monospace"}))
